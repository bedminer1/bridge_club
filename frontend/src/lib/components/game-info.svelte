...
<div class="flex flex-nowrap gap-x-2 sm:gap-x-4 text-2xs sm:text-xs text-muted-foreground px-1 overflow-x-auto scrollbar-none">
    ...
    <span class="whitespace-nowrap"><strong class="text-foreground font-medium">{playerName(game, game.BetWinner.ID)}</strong> + partner need <strong class="text-accent font-medium">{6 + game.BetSize}</strong> sets</span>
    ...
    <span>Winner <strong class="text-foreground font-medium">{playerName(game, game.BetWinner.ID)}</strong></span>
